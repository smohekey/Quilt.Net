namespace Quilt.Xml {
	...
	public class QuiltNamespace {
		private delegate QuiltElement CreateElementDelegate(string prefix, string localName, string namespaceURI, QuiltDocument document);
		...
		private static readonly Type[] __constructorTypes = new[] { __stringType, __stringType, __stringType, __documentType };
		...
		private static CreateElementDelegate CreateElementLambda(Type type) {
			ConstructorInfo? constructor = type.GetConstructor(BindingFlags.NonPublic | BindingFlags.Instance, null, __constructorTypes, null);

			var prefix = Expression.Prefix(__stringType, nameof(prefix));
			var localName = Expression.Prefix(__stringType, nameof(localName));
			var namespaceURI = Expression.Prefix(__stringType, nameof(namespaceURI));
			var document = Expression.Parameter(__documentType, nameof(document));
		...
		private QuiltNamespace() : this(URI) {
